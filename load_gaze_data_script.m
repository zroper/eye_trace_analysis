...
%load subject data
subj_texas = raw_gaze_in;
NUM_SUBJ = 1;%length(subj_texas.sleep);
...
%   end
  ...
  tgt = {'time','x','y'};%new_struct({'time','x','y'}, 'dim',[1,NUM_BLOCK]);
  gaze = {'time','x','y','vx','vy','v'};%new_struct({'time','x','y','vx','vy','v'}, 'dim',[1,NUM_BLOCK]);
  
  subj_id = 1;
  [gaze] = load_data_single_block([subj_id, 'test.csv'], 'test', raw_gaze_in);
...
function [ gaze ] = load_data_single_block( file_id , blk_type , dataset )

global ROOT_DIR SAMP_RATE B_FILT A_FILT NUM_TRIAL

data = dataset; %importdata([ROOT_DIR, 'raw/S_', file_id], '\t', 1);
